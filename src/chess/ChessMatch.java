...
public class ChessMatch {
	...
	private Piece makeMove(Position source, Position target) {
		ChessPiece attackingPiece = (ChessPiece) board.removePiece(source);
		...
		if(attackingPiece instanceof King && target.getColumn() == source.getColumn() - 2) {
			Position casSource = new Position(source.getRow(), source.getColumn() - 4);
			Position casTarget = new Position(casSource.getRow(), casSource.getColumn() + 2);
			makeMove(casSource, casTarget);			
		}
		
		// #Special Move Small Castling
		
		if(attackingPiece instanceof King && target.getColumn() == source.getColumn() + 2) {
			Position casSource = new Position(source.getRow(), source.getColumn() + 3);
			Position casTarget = new Position(casSource.getRow(), casSource.getColumn() - 2);
			makeMove(casSource, casTarget);			
		}
		...
		if(attackingPiece instanceof Pawn) {
			if(source.getColumn() != target.getColumn() && capturedPiece == null) {
				...
				if(attackingPiece.getColor() == Color.WHITE) {
					vulnerablePawnPosition = new Position(target.getRow() + 1, target.getColumn());
				}
				else {
					vulnerablePawnPosition = new Position(target.getRow() - 1, target.getColumn());
				}
				capturedPiece = board.removePiece(vulnerablePawnPosition);
				capturedPieces.add(capturedPiece);
				piecesOnTheBoard.remove(capturedPiece);			}
		}
	...
	private void undoMove(Position source, Position target, Piece capturedPiece) {
		...
		if (p instanceof King && target.getColumn() == source.getColumn() - 2) {
			Position casSource = new Position(source.getRow(), source.getColumn() - 4);
			Position casTarget = new Position(casSource.getRow(), casSource.getColumn() + 2);
			undoMove(casSource, casTarget, capturedPiece);
		}
		...
		if (p instanceof Pawn) {
			if (source.getColumn() != target.getColumn() && capturedPiece == enPassantVulnerable) {
				...
				board.placePiece(pawn, vulnerablePawnPosition);
			}
		}
